...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var otpEyeButton: UIButton!
    ...
    @IBAction func otpButtonTapped(_ sender: UIButton) {
        ...
        if sender.currentTitle == "OTP" {
            ...
            otpTF.isSecureTextEntry = false

            
            let starMark = NSAttributedString(string: "*", attributes: [.foregroundColor: UIColor.red])

            let mobileAtrributedString = NSMutableAttributedString(string: "One Time Password  ")
            mobileAtrributedString.append(starMark)
            otpTF.attributedPlaceholder = mobileAtrributedString
            
            
        }else {
            getOtpButton.isHidden = true
            otpEyeButton.isHidden = false
            otpButton.setTitle("OTP", for: .normal)
            otpTF.isSecureTextEntry = true
            
            let starMark = NSAttributedString(string: "*", attributes: [.foregroundColor: UIColor.red])

            let mobileAtrributedString = NSMutableAttributedString(string: "Password ")
            mobileAtrributedString.append(starMark)
            otpTF.attributedPlaceholder = mobileAtrributedString
        }
    ...
    @IBAction func selectionBox(_ sender: UIButton) {
        
        if sender.tag == 0 {
            // Preent Unbox
            sender.setImage(UIImage(named: "action selection"), for: .normal)
            sender.tag = 1
            
        } else {
            sender.setImage(UIImage(named: "selection box"), for: .normal)
            sender.tag = 0
    ...
    @IBAction func otpEyeButtonTapped(_ sender: UIButton) {
        if otpTF.isSecureTextEntry == true {
            otpTF.isSecureTextEntry = false
